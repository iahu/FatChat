...
var actions = {
	quit: redirect,
	showModal: showModal,
	addFriends: function (arg) {
		var $that = $(this);
		var $item = $(this).closest('.friend-item');
		if ($item.is('.active')) {
			return;
		}
		addFriends({ids: arg, uid: uid}).done(function (data) {
			if (data && data.success) {
				$item.addClass('active');
				$that.attr('disabled', 'disabled').text('已经添加')
			}
		})
	}
};
var friendsData, sessionsData;
function redirect() {
	deleteCookie('s');
	deleteCookie('P0');
	deleteCookie('P1');
...
.done(function () {
	getFriends({uid: uid}).done(getFriendsHandle);
});
...
$body.on('click active', '[data-action]', function(event) {
	...
	var arg = ($(this).data('arg') || '').toString().split(/\s?,\s?/);
...
function searchFriendsHandler(e) {
	...
	searchFriends({info: info, uid: uid}).done( function (data) {
		if (!(data && data.success)) {
			return;
		}

		$modal.filter('#addfriends-modal').find('.friends-list')
			.html( renderSearchFriends(data.msg) );
	} )
}
...
$body
.on('click', '#addfriends-modal .btn-primary', closeModal);
